	...
	<view class="details">
		<scroll-view scroll-y="">
			...
			<view class="xx">
				...
				<view class="sheng">
					<text class="iconfont iconhuo"><text>{{list.zxrd}}</text></text>
					<text class="iconfont iconsheng"><text>￥{{list.yj}}</text></text>
				</view>
			</view>
			<!-- 简介 书友圈 -->
			...
			<!-- 底部 -->
			<view class="footer">
				<view class="shoucang" @click="shoucang(list.id)" :class="flag1?'red':''" v-model="list.delete_flag">
					<text class="iconfont iconshoucang sctb"></text>
					<text class="scz">收藏</text>
				</view>
				<view class="jia">
					<text class="sbs">{{this.bao.length.toString()}}本</text>
					<text class="jrsb">加入书包</text>
				</view>
				<view class="jieyue">
					<text class="iconfont iconwodejieyue ljjetb"></text>
		...
		methods: {
			...
			async getSwiper() {
				...
				if (result != 0) {
					...
					this.list.zznrtwjj = this.list.zznrtwjj.replace(/\<img src="/gi, '<img style="max-width: 100%;" src="https://uptownlet.com');
